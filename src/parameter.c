...
void ParameterDestroy(HM_Hashmap *params) { hashmap_free(params); };

B8 ParameterIter(HM_Hashmap *params, U32 *iter, Parameter **parameter) {
    return hashmap_iter(params, iter, (void **)parameter);
}

Parameter *ParameterGet(HM_Hashmap *params, const char *name) {
    return (Parameter *)hashmap_get(params, &(Parameter){.name = name});
}

F32 ParameterGetValue(HM_Hashmap *params, const char *name) {
    Parameter *param =
...
_Parameter ParameterSet(HM_Hashmap *params, Parameter *param) {
    hashmap_set(params, param);

    return ParameterGeneratePtr(params, param->name);
}

void ParameterSetValue(HM_Hashmap *params, const char *name, F32 value) {
...
U32 ParameterCount(HM_Hashmap *params) { return hashmap_count(params); }

void _ParameterSetValue(_Parameter param, F32 value) {
    ParameterSetValue(param.parameters, param.name, value);
}

F32 _ParameterGetValue(_Parameter param) {
    return ParameterGetValue(param.parameters, param.name);
}

_Parameter ParameterGeneratePtr(HM_Hashmap *params, const char *name) {
