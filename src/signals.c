...
// NULL for *out_frequencies e.g signals_process_samples(LOG_MUL, START_FREQ, 0,
  ...
  *out_frequency_count =
  ...
  if (out_frequencies == NULL) {
    for (u32 i = 0; i < smoothing; ++i) {
      signals_smooth_convolve(NULL, *out_frequency_count, NULL, filter_count,
                              NULL, out_frequency_count);
    }
    return;
  }
  ...
  for (u32 i = 0; i < smoothing; ++i) {
    signals_smooth_convolve(log_freq, *out_frequency_count, filter,
  ...
  for (u32 i = 0; i < *out_frequency_count; ++i) {
    if (log_freq[i] > -FLT_MAX) {
      smooth_frequencies[i] += 100 * (log_freq[i] - smooth_frequencies[i]) * dt;
    }
  }
