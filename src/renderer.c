...
static inline void draw_frequency_polygon(Texture2D tex, HMM_Vec2 *indices,
                                          u32 index_count, HMM_Vec2 *vertices,
...
void renderer_draw_waveform(Renderer *renderer, u32 sample_count, f32 *samples,
  ...
  for (u32 i = 0; i < count; ++i) {
    ...
    i32 index =
        (i32)(ceilf(sample_count - a * sample_count / wave_width_multiplier));

    f32 t = samples[index];

    if (t > 0) {
      DrawRectangle(i * cell_width,
                    (renderer->render_size.Height / 2) * (1 - t), cell_width,
                    renderer->render_size.Height / 2 * t, RED);
    } else {
...
void renderer_draw_frequencies(Renderer *renderer, u32 frequency_count,
  ...
  for (u32 i = 0; i < vertex_count; ++i) {
    ...
    colors[i] = color;
    vertices[i] =
        HMM_V2(i * cell_width, renderer->render_size.Height * (1 - 0.5 * t));
  }
  ...
  for (u32 i = 0; i < vertex_count; ++i) {
    ...
    indices[i] =
        HMM_SubV2(vertex, HMM_V2(renderer->render_size.Width / 2.f,
  ...
  draw_frequency_polygon(renderer->textures[Textures_DEFAULT], indices,
