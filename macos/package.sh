...
clang ${SRC} -Wall -Wextra -Wno-unused-parameter -Wno-unused-but-set-variable -Wno-pointer-type-mismatch -D PLAT_MACOS -O3 -o ./Vizzy.app/Contents/MacOS/Vizzy ${CFLAGS} ${LDFLAGS}
echo "clang ${SRC} -Wall -Wextra -Wno-unused-parameter -Wno-unused-but-set-variable -Wno-pointer-type-mismatch -D PLAT_MACOS -O3 -o ./Vizzy.app/Contents/MacOS/Vizzy ${CFLAGS} ${LDFLAGS}"
mkdir -p Vizzy.app/Contents/Frameworks/

echo -e ${BLUE}Creating icon${NC}
./macos/createicn.sh ./assets/logo.png ./Vizzy.app/Contents/Resources/icon.iconset ./Vizzy.app/Contents/Resources/Vizzy.icns

echo -e ${BLUE}Copying libraries...${NC}
cp -r /usr/local/lib/libcurl.4.dylib Vizzy.app/Contents/Frameworks/
cp -r /usr/local/lib/libportaudio.2.dylib Vizzy.app/Contents/Frameworks/
cp -r /usr/local/lib/libportaudio.dylib Vizzy.app/Contents/Frameworks/
cp -r /opt/homebrew/opt/libnghttp2/lib/libnghttp2.14.dylib Vizzy.app/Contents/Frameworks/
cp -r /opt/homebrew/opt/libidn2/lib/libidn2.0.dylib Vizzy.app/Contents/Frameworks/
cp -r /opt/homebrew/opt/openssl@3/lib/libssl.3.dylib Vizzy.app/Contents/Frameworks/
cp -r /opt/homebrew/opt/openssl@3/lib/libcrypto.3.dylib Vizzy.app/Contents/Frameworks/
cp -r /opt/homebrew/opt/libunistring/lib/libunistring.5.dylib Vizzy.app/Contents/Frameworks/
cp -r /opt/homebrew/opt/gettext/lib/libintl.8.dylib Vizzy.app/Contents/Frameworks/
cp -r build/assets Vizzy.app/Contents/Resources/
cp -r build/lua Vizzy.app/Contents/Resources/
cp -r macos/Info.plist Vizzy.app/Contents/
...
echo -e ${BLUE}Creating Vizzy.dmg...${NC}
